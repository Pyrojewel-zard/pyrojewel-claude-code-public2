#!/usr/bin/env node
/**
 * PostToolUse Hook: Accumulate edited Python file paths for batch processing
 *
 * Cross-platform (Windows, macOS, Linux)
 *
 * Records each edited Python path to a session-scoped temp file (one path per
 * line). stop-format-typecheck.js reads this list at Stop time and runs ruff
 * format + ruff check once across all edited files, eliminating per-edit latency.
 *
...
function appendPath(filePath) {
  if (filePath && PY_EXT.test(filePath)) {
