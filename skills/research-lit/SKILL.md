...
> - `/research-lit "topic" — sources: ieee` — IEEE 优先：ieee-lit-search → arxiv → lit-reading
> - `/research-lit "topic" — sources: ieee, zotero` — IEEE + Zotero（合并去重）
...
- **If `— sources:` is specified**: Only search the listed sources (comma-separated). Valid values: `ieee`, `zotero`, `obsidian`, `local`, `web`, `semantic-scholar`, `deepxiv`, `exa`, `gemini`, `openalex`, `all`.
...
|----------|--------|----|---------------|-----------------|
| 0 | **IEEE Xplore** | `ieee` | `ieee-lit-search` skill exists | IEEE 期刊/会议论文，多轮检索，核心期刊过滤 |
...
### Step 0: IEEE 优先模式（如果 `--sources: ieee`）

当用户指定 `--sources: ieee` 时：

1. **IEEE 检索**：
   ```
   ieee_papers = /ieee-lit-search {query} --venues {venues} --year-range {year_range}
   ```

2. **arXiv 补充**：
   ```
   arxiv_papers = arxiv search {query}
   ```

3. **去重合并**：
   - 按 DOI/arXiv ID/normalized title 三级去重
   - 合并 IEEE 和 arXiv 结果

4. **分级阅读**：
   ```
   /lit-reading {papers} --shallow-read-max 20 --deep-read-top 5
   ```

5. **返回 LIT_REVIEW.md 路径**

**跳过 Step 0a-0c**，直接进入 Step 1（如果需要更多来源）。
