...
- 综述输入若带精读笔记，先做严格 TSV 的“章节—页面—证据”覆盖矩阵；笔记每个二级/三级标题都必须有映射，不能只按图片数量判断完整度。
- 芯片案例页固定呈现“工作 / 对象 / 自动化边界 / 证据层级 / 未覆盖项”，并区分原文事实和跨文献归纳。
...
综述不应把章节顺序直接翻译成页面顺序。至少安排一页 `causal_chain`，说明表示方式如何改变搜索空间、搜索空间如何改变数据预算、验证结果如何回流采样或模型；至少安排一页 `case_boundary`，对照代表工作做到的自动化层级和仍保留的人工环节；结尾安排一页 `synthesis_boundary`，分别列出综述支持、不能支持和未解决的判断。
