...
> **注意**：具体的 profile 配置（端口、用途、tech lib、CIW daemon 加载路径）属于用户隐私信息，存储在 memory 中。使用 skill 时，通过 `[[virtuoso-bridge-profiles]]` 和 `[[virtuoso-bridge-ciw-load-commands]]` 引用。

**通用连接模式**：
```bash
# CLI — 通过 -p 指定 profile
virtuoso-bridge start -p <profile>
virtuoso-bridge status -p <profile>

# Python
client = VirtuosoClient.from_env(profile="<profile>")
```

**查找 profile 信息**：
- 查看可用 profile 列表：`virtuoso-bridge status`（不带 -p 时显示所有）
- profile 详情（端口、用途、tech lib）：见 `[[virtuoso-bridge-profiles]]`
- CIW daemon 加载命令：见 `[[virtuoso-bridge-ciw-load-commands]]`

多 profile 可并行运行，各自独立端口互不干扰。daemon 已运行时 `start` 不会替换它。
