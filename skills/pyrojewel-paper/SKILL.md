...
先判断这篇论文的主类型，决定阅读重心——不要对每篇论文都用同一套模板：

| 类型 | 特征 | 阅读重心 |
|------|------|---------|
| **解释型** | 提出机制/理论，解释现象 | 概念如何共同生成现象，条件变化时模型怎样失效 |
| **方法型** | 提出新方法/模型 | 方法机制、关键设计选择、与 baseline 的实质差异 |
| **测量型** | 提出新 benchmark/评测 | 新 benchmark 让什么问题变得可测（不等于解释了现实世界） |
| **资源型** | 数据集/工具/系统 | 适用范围、质量验证、能解锁什么下游工作 |
| **理论型** | 定理/证明/数学结果 | 假设链、适用边界、反例与边界案例 |

评测、数据集和工具论文常常只是暴露旧尺子的盲点——不要硬造稻草人（"这篇在反对旧观念"）。评测贡献不等于科学发现。

### 3.6 概念缺口账本（核心概念的系统化扫描）

「见过术语」不等于「已经有这个概念」。进入「核心概念」拆解前，先建立内部的**概念缺口账本**：围绕暂定中心命题，扫描命题、机制和证据链上每个承重词，判断读者是否真正拥有它——若读者不能完成下面任一件事，就记入缺口：

- 用普通话指出它**区分了什么**，以及它**不是什么**；
- 说明它**依赖什么、影响什么**，和相邻概念怎样连接；
- 在论文案例中**认出它**，并判断一个反例或边界案例为何不同。

按**依赖顺序**补：先补后续概念所依赖的，再补论文新增或改写的。概念缺口不是术语表——只记录中心命题、机制和证据链上的承重概念；外围术语按需解释，不让清单淹没论文思想。
...
### 5.5 最小解释模型：把概念接成能运行的模型

核心概念逐个解锁后，把它们接成一个**最小解释模型**——不是流程图的别名，是能解释原锚点、区分邻近情况、并对条件变化给出方向性预测的模型：
...
找出最大的剩余阻力：读者可能仍无法在脑中形成关系，也可能无法判断证据究竟支持模型到哪一步。选择理解成本最低的表征，不要默认继续写 prose：
