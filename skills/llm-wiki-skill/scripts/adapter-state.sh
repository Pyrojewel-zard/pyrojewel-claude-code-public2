...
default_install_hint() {
  ...
  case "$source_id" in
    web_article|x_twitter|zhihu_article)
      ...
      ;;
    wechat_article)
      printf '%s\n' "先安装 uv，再执行：uv tool install ${WECHAT_TOOL_URL}；同时确认 wechat-article-extract skill 已准备到技能目录"
      ;;
    youtube_video)
      printf '%s\n' "重新运行当前平台的 llm-wiki 安装命令，并追加 --with-optional-adapters，确认 ${adapter_name} 已准备到技能目录"
      ;;
    xiaohongshu_post)
      printf '%s\n' "重新运行当前平台的 llm-wiki 安装命令，并追加 --with-optional-adapters，确认 ${adapter_name} 已准备到技能目录；然后按照 SKILL.md 说明完成小红书登录配置"
      ;;
    *)
      printf '%s\n' "-"
      ;;
...
optional_hint() {
  ...
  case "$source_id" in
    ...
    wechat_article|youtube_video)
      ...
      ;;
    xiaohongshu_post)
      printf '%s\n' "先安装 uv：brew install uv；然后按照 xiaohongshu-extract SKILL.md 说明完成小红书登录配置"
      ;;
    *)
      printf '%s\n' "-"
      ;;
      ...
      case "$source_id" in
        ...
        xiaohongshu_post)
          if ! dependency_installed "$dependency_name" "$dependency_type"; then
            ...
            state="env_unavailable"
            detail="缺少 uv，当前无法运行小红书帖子提取"
            recovery_action="先补环境；现在也可以直接走手动入口"
            install_hint="$(optional_hint "$source_id")"
          else
            local optional_root
            optional_root="$(resolve_optional_root)"
            if [ ! -f "$HOME/.local/share/xiaohongshu-extract/xhs_state.json" ]; then
              state="env_unavailable"
              detail="小红书登录态未配置（缺少 xhs_state.json）"
              recovery_action="先完成小红书登录；现在也可以直接走手动入口"
              install_hint="按照 xiaohongshu-extract SKILL.md 说明完成首次登录配置"
            else
              state="available"
              detail="${adapter_name} 已可用"
              recovery_action="继续自动提取"
