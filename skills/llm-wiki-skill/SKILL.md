...
- `wechat-article-extract` — 微信公众号提取（底层依赖 wechat-article-to-markdown）
- `youtube-transcript` — YouTube 字幕提取
- `xiaohongshu-extract` — 小红书帖子提取（需要登录态配置）
...
8. **输出引导**（根据 `WIKI_LANG` 切换语言）：
   ...
   **中文（zh）**：
   ...
   - 小红书帖子可以自动提取（需先配置登录态），也可以直接粘贴文本给我
...
- 如果 `adapter_name=wechat-article-extract` → 执行 `wechat-article-to-markdown "<URL>"`（底层命令）
