...
输入：研究方向关键词
  ...
  │
  ├─ Step 4: 组会汇报PPT
  │     /pyrojewel-beamer-academic ── 生成完整调研Beamer PPT
  │     输入：river输出（框架）+ paper输出×N（细节）
  │     输出：presentation.tex + presentation.pdf + assets
  │     上游替代：原 /beamer-academic 已由 pyrojewel-beamer-academic 正式替代
  │
  │     PPT结构设计：
  │       ├─ 开场(1页)：调研方向说明
  │       ├─ 框架页（来自river）：
  ...
  │       └─ 下一步计划(1页)
  │
  │     注：beamer-academic当前设计是从论文PDF生成PPT，
  │         此场景需从多个skill的markdown输出生成PPT——
  │         可能需要适配输入方式，或先整理成"调研大纲"再输入
  │
  └─ Step 5: idea判断与提炼
        阅读自己产出的笔记 + 组会PPT后的思考
        形成对方向的独立判断
...
| pyrojewel-beamer-academic | pyrojewel-beamer-academic | 1, 11 | 即用(正式替代beamer-academic) |
| pyrojewel-academic-ppt | .claude/skills | 11 | 待统一(与pyrojewel-beamer-academic重叠) |
| beamer-academic | beamer-academic | 1, 11 | 已被pyrojewel-beamer-academic替代(仅参考) |
