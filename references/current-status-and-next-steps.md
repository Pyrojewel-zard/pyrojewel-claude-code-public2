...
2. **论文阅读主线已落地核心 skill**
   - 已落地：`pyrojewel-paper`、`pyrojewel-paper-river`、`pyrojewel-paper-qa`
   - 已落地：`pyrojewel-paper-flow`
   - 已落地：`zotero-pdf-parse`
   - 已落地：`pyrojewel-beamer-academic`
   - 当前只差环境验证，不差链路设计
...
- `pyrojewel-beamer-academic` 正式替代原 `beamer-academic`
- `pyrojewel-academic-ppt` 视为 superseded，只保留参考意义
...
- `pyrojewel-paper-flow` 已从占位变成 7-phase 编排器
- `pyrojewel-beamer-academic` 已被确定为正式 PPT skill
...
3. `pyrojewel-paper-flow` 还没有完成一次完整闭环实测
...
5. `pyrojewel-beamer-academic` 正式替代 `beamer-academic` 和 `pyrojewel-academic-ppt` 的判断是否完全成立
...
5. `pyrojewel-beamer-academic` 正式替代 `beamer-academic` 和 `pyrojewel-academic-ppt`：**同意**
...
- 已完成：
  - `pyrojewel-paper-flow` 中 `zotero-pdf-parse` 路径环境变量化
  - 论文主线文档同步更新
