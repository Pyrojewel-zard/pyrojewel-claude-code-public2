 ...
 * Added: Obsidian vault sync on Stop.
 *
 ...
 * session file for cross-session continuity. Optionally syncs to Obsidian vault.
 */
...
`;
  ...
  // Sync session file to Obsidian vault (if configured)
  syncToObsidian(sessionFile, sessionMetadata.project);

  // Sync learnings to Obsidian vault
  syncLearningsToObsidian(sessionMetadata.project);
...
/**
 * Sync the session file to an Obsidian vault for long-term knowledge archival.
 * Controlled by PJ_OBSIDIAN_VAULT env var (path to vault root).
 * Files are written to <vault>/Claude Sessions/<project>/ directory.
 */
function syncToObsidian(sessionFile, projectName) {
  const vaultRoot = process.env.PJ_OBSIDIAN_VAULT;
  if (!vaultRoot) return;

  if (!fs.existsSync(vaultRoot)) {
    log(`[SessionEnd] Obsidian vault not found: ${vaultRoot}`);
    return;
  }

  if (!fs.existsSync(sessionFile)) return;

  const safeProject = (projectName || 'unknown').replace(/[^a-zA-Z0-9_-]/g, '_');
  const obsidianDir = path.join(vaultRoot, 'inbox', 'session_log', safeProject);
  ensureDir(obsidianDir);

  const destFile = path.join(obsidianDir, path.basename(sessionFile).replace(/\.tmp$/, '.md'));

  try {
    const content = readFile(sessionFile);
    if (content) {
      writeFile(destFile, content);
      log(`[SessionEnd] Synced to Obsidian: ${destFile}`);
    }
  } catch (err) {
    log(`[SessionEnd] Obsidian sync failed: ${err.message}`);
  }
}

/**
 * Sync project learnings to Obsidian vault at inbox/continue_learning/<project>/.
 * Each .yml/.yaml learning file is converted to a markdown file for readability.
 */
function syncLearningsToObsidian(projectName) {
  const vaultRoot = process.env.PJ_OBSIDIAN_VAULT;
  ...
  const safeProject = (projectName || 'unknown').replace(/[^a-zA-Z0-9_-]/g, '_');
  const obsidianDir = path.join(vaultRoot, 'inbox', 'continue_learning', safeProject);
  ensureDir(obsidianDir);
  ...
  try {
    ...
    for (const file of files) {
      ...
      // Convert YAML learning to readable markdown
      ...
      const id = idMatch ? idMatch[1].trim() : file.replace(/\.\w+$/, '');
      ...
      const destFile = path.join(obsidianDir, `${id}.md`);
      writeFile(destFile, md);
      synced++;
    }

    if (synced > 0) {
      log(`[SessionEnd] Synced ${synced} learning(s) to Obsidian: ${obsidianDir}`);
    }
  } catch (err) {
    log(`[SessionEnd] Learnings Obsidian sync failed: ${err.message}`);
  }
}
...
function writeSessionContext(metadata, summary) {
  ...
  if (existing.activeThreads) {
    lines.push(existing.activeThreads);
  } else {
